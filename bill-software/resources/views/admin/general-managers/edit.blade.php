...
@section('title', 'Edit General Manager')
    ...
    <div class="row">
        <div class="col-12">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h2 class="mb-0">Edit General Manager</h2>
            ...
            <div class="card shadow-sm">
                ...
                <div class="card-body">
                    <form action="{{ route('admin.general-managers.update', $generalManager) }}" method="POST">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control @error('name') is-invalid @enderror" 
                                           id="name" name="name" value="{{ old('name', $generalManager->name) }}" 
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control @error('code') is-invalid @enderror" 
                                           id="code" name="code" value="{{ old('code', $generalManager->code) }}" 
                            ...
                            <div class="col-md-12">
                                <div class="mb-3">
                                    ...
                                    <textarea class="form-control @error('address') is-invalid @enderror" 
                                              ...
                                              placeholder="Enter address">{{ old('address', $generalManager->address) }}</textarea>
                                    @error('address')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control @error('telephone') is-invalid @enderror" 
                                           id="telephone" name="telephone" value="{{ old('telephone', $generalManager->telephone) }}" 
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control @error('mobile') is-invalid @enderror" 
                                           id="mobile" name="mobile" value="{{ old('mobile', $generalManager->mobile) }}" 
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="email" class="form-control @error('email') is-invalid @enderror" 
                                           id="email" name="email" value="{{ old('email', $generalManager->email) }}" 
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control @error('status') is-invalid @enderror" 
                                           id="status" name="status" value="{{ old('status', $generalManager->status) }}" 
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="mb-3">
                                    <label for="dc_mgr" class="form-label">D.c.mgr <span class="text-danger">*</span></label>
                                    <div class="input-group">
                                        <input type="text" class="form-control @error('dc_mgr') is-invalid @enderror" 
                                               id="dc_mgr" name="dc_mgr" value="{{ old('dc_mgr', $generalManager->dc_mgr) }}" 
                                               placeholder="Select divisional manager" readonly>
                                        <button class="btn btn-outline-primary" type="button" onclick="openDivisionalManagerModal()">
                        ...
                        <div class="d-flex gap-2">
                            <button type="submit" class="btn btn-primary">
                                <i class="bi bi-check-lg me-2"></i>Update General Manager
                            </button>
...
<div id="divisionalManagerModal" class="manager-modal">
    <div class="manager-modal-content">
        <div class="manager-modal-header">
            <h5 class="manager-modal-title"><i class="bi bi-people me-2"></i>Select Divisional Manager</h5>
            <button type="button" class="btn-close-modal" onclick="closeDivisionalManagerModal()"><i class="bi bi-x-lg"></i></button>
        </div>
        <div class="manager-modal-body" id="divisionalManagerModalBody"></div>
    </div>
</div>
...
function openDivisionalManagerModal() {
    ...
    fetch('{{ route("admin.divisional-managers.index") }}?modal=true', {
        ...
        .then(data => populateDivisionalManagerModal(data))
        .catch(error => { console.error('Error:', error); modalBody.innerHTML = `<div class="alert alert-danger m-3"><i class="bi bi-exclamation-triangle me-2"></i>Failed to load divisional managers.</div>`; });
}
function populateDivisionalManagerModal(managers) {
    const modalBody = document.getElementById('divisionalManagerModalBody');
    if (!managers || managers.length === 0) {
        modalBody.innerHTML = `<div class="text-center py-5"><i class="bi bi-inbox" style="font-size: 3rem; color: #ccc;"></i><p class="text-muted mt-3">No divisional managers found</p><a href="{{ route('admin.divisional-managers.create') }}" class="btn btn-sm btn-primary mt-2"><i class="bi bi-plus-lg me-2"></i>Add New</a></div>`;
        return;
    }
    let html = '<div class="row g-2">';
    managers.forEach(manager => {
        html += `<div class="col-12"><div class="manager-card p-3" onclick="selectDivisionalManager('${manager.name} (${manager.code})', '${manager.id}')"><div class="d-flex justify-content-between align-items-start"><div><h6 class="mb-1 fw-600">${manager.name}</h6><small class="text-muted">Code: ${manager.code}</small></div><i class="bi bi-chevron-right text-primary"></i></div><div class="mt-2 small text-muted"><div><i class="bi bi-telephone me-2"></i>${manager.mobile || 'N/A'}</div><div><i class="bi bi-envelope me-2"></i>${manager.email || 'N/A'}</div></div></div></div>`;
    });
    html += '</div>';
    modalBody.innerHTML = html;
}
function selectDivisionalManager(name, id) { document.getElementById('dc_mgr').value = name; closeDivisionalManagerModal(); }
...
document.addEventListener('click', function (e) { if (e.target && e.target.id === 'divisionalManagerModalBackdrop') closeDivisionalManagerModal(); });
document.addEventListener('keydown', function (e) { if (e.key === 'Escape') { const modal = document.getElementById('divisionalManagerModal'); if (modal && modal.classList.contains('show')) closeDivisionalManagerModal(); } });
</script>
<style>
