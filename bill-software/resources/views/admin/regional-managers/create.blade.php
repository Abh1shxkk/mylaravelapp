    ...
    <div class="row">
        <div class="col-12">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h2 class="mb-0">Create New Regional Manager</h2>
            ...
            <div class="card shadow-sm">
                ...
                <div class="card-body">
                    <form action="{{ route('admin.regional-managers.store') }}" method="POST">
                        @csrf
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="mb-3">
                                    <label for="mkt_mgr" class="form-label">Mkt.mgr.</label>
                                    <div class="input-group">
                                        <input type="text" class="form-control @error('mkt_mgr') is-invalid @enderror" 
                                               id="mkt_mgr" name="mkt_mgr" value="{{ old('mkt_mgr') }}" 
                                               placeholder="Enter marketing manager" readonly>
                                        <button type="button" class="btn btn-outline-primary" onclick="openMarketingManagerModal()">
                        ...
                        <div class="d-flex gap-2">
                            <button type="submit" class="btn btn-primary">
                                <i class="bi bi-check-lg me-2"></i>Create Regional Manager
                            </button>
...
<!-- Marketing Manager Selection Modal -->
    ...
    <div class="manager-modal-content">
        <div class="manager-modal-header">
            <h5 class="manager-modal-title">
                <i class="bi bi-person-badge me-2"></i>Select Marketing Manager
            </h5>
...
function openMarketingManagerModal() {
    ...
    .then(data => {
        populateMarketingManagerData(data);
    })
    .catch(error => {
        console.error('Error:', error);
        showErrorInMarketingManagerModal('Failed to load marketing managers. Please try again.');
    });
}

function populateMarketingManagerData(managers) {
    const modalBody = document.getElementById('marketingManagerModalBody');
    ...
    if (!managers || managers.length === 0) {
        modalBody.innerHTML = `
            <div class="text-center py-4">
                ...
                <p class="text-muted">No marketing managers found</p>
                <a href="{{ route('admin.marketing-managers.create') }}" class="btn btn-primary btn-sm">
                    <i class="bi bi-plus-lg me-1"></i>Add Marketing Manager
                </a>
            </div>
        `;
        return;
    ...
    managers.forEach(manager => {
        html += `
            <div class="col-12">
                <div class="card manager-card" onclick="selectMarketingManager('${manager.name}', '${manager.code || ''}')">
...
function selectMarketingManager(name, code) {
    const mktMgrInput = document.getElementById('mkt_mgr');
    mktMgrInput.value = name + (code ? ' (' + code + ')' : '');
    closeMarketingManagerModal();
}

function showErrorInMarketingManagerModal(message) {
    const modalBody = document.getElementById('marketingManagerModalBody');
    modalBody.innerHTML = `
        <div class="text-center py-4">
            ...
            <button class="btn btn-outline-primary btn-sm" onclick="openMarketingManagerModal()">
