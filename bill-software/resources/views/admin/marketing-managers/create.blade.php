    ...
    <div class="row">
        <div class="col-12">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h2 class="mb-0">Create New Marketing Manager</h2>
            ...
            <div class="card shadow-sm">
                ...
                <div class="card-body">
                    <form action="{{ route('admin.marketing-managers.store') }}" method="POST">
                        @csrf
                        ...
                        <div class="d-flex gap-2">
                            <button type="submit" class="btn btn-primary">
                                <i class="bi bi-check-lg me-2"></i>Create Marketing Manager
                            </button>
...
<!-- General Manager Selection Modal -->
    ...
    <div class="manager-modal-content">
        <div class="manager-modal-header">
            <h5 class="manager-modal-title">
                <i class="bi bi-person-badge me-2"></i>Select General Manager
            </h5>
...
function openGeneralManagerModal() {
    ...
    .then(data => {
        populateGeneralManagerData(data);
    })
    .catch(error => {
        console.error('Error:', error);
        showErrorInGeneralManagerModal('Failed to load general managers. Please try again.');
    });
}

function populateGeneralManagerData(managers) {
    const modalBody = document.getElementById('generalManagerModalBody');
    ...
    if (!managers || managers.length === 0) {
        modalBody.innerHTML = `
            <div class="text-center py-4">
                ...
                <p class="text-muted">No general managers found</p>
                <a href="{{ route('admin.general-managers.create') }}" class="btn btn-primary btn-sm">
                    <i class="bi bi-plus-lg me-1"></i>Add General Manager
                </a>
            </div>
        `;
        return;
    ...
    managers.forEach(manager => {
        html += `
            <div class="col-12">
                <div class="card manager-card" onclick="selectGeneralManager('${manager.name}', '${manager.code || ''}')">
...
function selectGeneralManager(name, code) {
    const genMgrInput = document.getElementById('gen_mgr');
    genMgrInput.value = name + (code ? ' (' + code + ')' : '');
    closeGeneralManagerModal();
}

function showErrorInGeneralManagerModal(message) {
    const modalBody = document.getElementById('generalManagerModalBody');
    modalBody.innerHTML = `
        <div class="text-center py-4">
            ...
            <button class="btn btn-outline-primary btn-sm" onclick="openGeneralManagerModal()">
