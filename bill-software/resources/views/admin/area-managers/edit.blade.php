                ...
                <div class="card-body">
                    <form action="{{ route('admin.area-managers.update', $areaManager) }}" method="POST">
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="mb-3">
                                    <label for="reg_mgr" class="form-label">Reg.mgr. <span class="text-danger">*</span></label>
                                    <div class="input-group">
                                        <input type="text" class="form-control @error('reg_mgr') is-invalid @enderror" 
                                               ...
                                               placeholder="Select regional manager" readonly>
                                        <button class="btn btn-outline-primary" type="button" onclick="openRegionalManagerModal()">
    ...
    <div class="manager-modal-content">
        <div class="manager-modal-header">
            <h5 class="manager-modal-title">
                <i class="bi bi-people me-2"></i>Select Regional Manager
            </h5>
            <button type="button" class="btn-close-modal" onclick="closeRegionalManagerModal()">
...
function openRegionalManagerModal() {
    ...
    fetch('{{ route("admin.regional-managers.index") }}?modal=true', {
        ...
        .then(data => {
            populateRegionalManagerModal(data);
        })
        .catch(error => {
            ...
            modalBody.innerHTML = `
                <div class="alert alert-danger m-3">
                    ...
                    Failed to load regional managers. Please try again.
                </div>
            `;
        });
}

function populateRegionalManagerModal(managers) {
    const modalBody = document.getElementById('regionalManagerModalBody');
    ...
    if (!managers || managers.length === 0) {
        modalBody.innerHTML = `
            <div class="text-center py-5">
                ...
                <p class="text-muted mt-3">No regional managers found</p>
                <a href="{{ route('admin.regional-managers.create') }}" class="btn btn-sm btn-primary mt-2">
                    <i class="bi bi-plus-lg me-2"></i>Add New Regional Manager
                </a>
            </div>
        `;
        return;
    ...
    managers.forEach(manager => {
        html += `
            <div class="col-12">
                <div class="manager-card p-3" onclick="selectRegionalManager('${manager.name} (${manager.code})', '${manager.id}')">
...
function selectRegionalManager(name, id) {
    document.getElementById('reg_mgr').value = name;
